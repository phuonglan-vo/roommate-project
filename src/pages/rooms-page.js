...
const STATUS_BADGE_CLASSES = Object.freeze({
  [ROOM_STATUS.VACANT]: 'rm-room-badge--vacant',
  [ROOM_STATUS.OCCUPIED]: 'rm-room-badge--occupied',
  [ROOM_STATUS.MAINTENANCE]: 'rm-room-badge--maintenance',
  [ROOM_STATUS.INACTIVE]: 'rm-room-badge--inactive'
});
...
function createStatusBadge(status) {
  const label =
    ROOM_STATUS_LABELS[status] ?? 'Không xác định';
...
function createTableCell(label, child, className = '') {
...
function createDetailItem(label, value) {
  const term = createElement('dt', {
    className: 'col-sm-5 text-body-secondary',
    text: label
  });
...
function createRoomDetailDialog() {
  ...
  const body = createElement('div', {
    ...
    dataset: {
      testid: 'room-detail-content'
    }
  });

  const footerCloseButton = createElement('button', {
  ...
  const element = createElement(
    'div',
    {
      ...
      dataset: {
        testid: 'room-detail-modal'
      }
    },
    [dialog]
  );

  function open(room, occupancy) {
    ...
    const detailRows = [
      ...
      ['Loại phòng', getRoomTypeLabel(room.roomType)],
      [
        'Diện tích',
        room.areaM2
          ? `${room.areaM2} m²`
      ...
      [
        'Giá thuê',
        formatVietnameseCurrency(room.monthlyRent)
      ],
      [
        'Số người',
      ...
      [
        'Chỗ còn trống',
        String(occupancy.availableSpots)
      ],
      [
        'Mô tả',
    ...
    detailRows.forEach(([label, value]) => {
      details.append(...createDetailItem(label, value));
    });
    ...
    body.replaceChildren(details, statusRow);
...
export function createRoomsPage() {
  ...
  headingText.append(
    ...
    createElement('p', {
      ...
      text: 'Theo dõi trạng thái, giá thuê và sức chứa của các phòng.'
    })
  );
  ...
  Object.values(ROOM_STATUS).forEach((status) => {
    statusFilter.append(
      createElement('option', {
        text: ROOM_STATUS_LABELS[status],
        attributes: {
          value: status
        }
      })
  ...
  const sortSelect = createElement('select', {
    ...
    dataset: {
      testid: 'room-price-sort'
    }
  });

  sortSelect.append(
    createElement('option', {
      text: 'Sắp xếp mặc định',
      attributes: {
        value: ''
      }
    }),
  ...
  const tableWrapper = createElement(
    'div',
    {
      className: 'table-responsive rm-rooms-table-wrapper'
    },
    [table]
  );
  ...
  const emptyDescription = createElement('p', {
    ...
    text: 'Hãy thêm phòng đầu tiên để bắt đầu quản lý.'
  });
  ...
  const emptyState = createElement(
    'div',
    {
      className: 'rm-rooms-empty',
      attributes: {
        hidden: ''
      },
      dataset: {
        testid: 'rooms-empty-state'
      }
    },
    [
      createElement('div', {
        className: 'rm-rooms-empty-icon',
        text: '▦',
        attributes: {
          'aria-hidden': 'true'
        }
    ...
    async onSubmit(data, context) {
      if (context.mode === 'edit') {
        ...
        showToast({
          ...
          message: `Đã cập nhật phòng ${updatedRoom.code}.`
        });
      } else {
        const createdRoom =
        ...
        showToast({
          type: 'success',
          title: 'Thêm phòng',
          message: `Đã thêm phòng ${createdRoom.code}.`
        });
      }
  ...
  function refreshRooms() {
    try {
      state.rooms = roomService.getRooms();
      renderTable();
    } catch (error) {
      state.rooms = [];
      renderTable();
  ...
  async function deleteRoom(roomId) {
    ...
    if (!room) {
      showToast({
        ...
        message: 'Phòng cần xóa không còn tồn tại.'
      });
    ...
    try {
      ...
      showToast({
        type: 'success',
        title: 'Xóa phòng',
        message: `Đã xóa phòng ${room.code}.`
      });
  ...
  tableBody.addEventListener('click', (event) => {
    ...
    if (!room) {
      showToast({
        type: 'danger',
        title: 'Không tìm thấy phòng',
        message: 'Dữ liệu phòng không còn tồn tại.'
      });
...
export const createPage = createRoomsPage;
